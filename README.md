...
This repository is a **standalone demo HTTP API** intended for use with the **[API Testing learning path](https://academy.postman.com/path/api-testing-path)** on Postman Academy (Postman’s API Testing courses). It is **designed to run on your machine** so you can send requests, write tests in Postman, and practice workflows without depending on a shared hosted environment.
